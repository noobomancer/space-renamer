...
- Parsed by the pure `SpacesPlistParser` into `ParsedSpace { id: String, ordinal: Int }` + `activeID: String?`. `ParsedSpace` also exposes `static let maxShortcutOrdinal = 9` and `var isShortcutAvailable: Bool` — the single source of truth for the ">9 Spaces have no Ctrl+digit" rule, which `SwitcherEngine`'s ordinal guard derives from too (so the menu never re-implements it). `SpaceMonitor` exposes `@Published spaces: [ParsedSpace]`, `@Published activeID: String?`, `@Published lastLoadError: String?` (`nil` = healthy; non-`nil` = the last read/parse failed and `spaces`/`activeID` are retained stale), and `ordinal(for id: String) -> Int?`.
